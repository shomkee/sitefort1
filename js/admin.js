...
function saveConfig() {
  const config = {
    url: document.getElementById('cfgUrl').value.trim() || 'test.zip',
    name: document.getElementById('cfgName').value.trim() || 'test.zip',
    size: document.getElementById('cfgSize').value.trim() || '24 MB',
    password: document.getElementById('cfgPassword').value.trim() || 'test'
  };
...
function testDownload() {
  const url = document.getElementById('cfgUrl').value.trim() || 'test.zip';
  window.open(url, '_blank');
}

function loadConfig() {
  const cfg = localStorage.getItem('test_config');
  if (cfg) {
    try {
  ...
  const pwd = localStorage.getItem('test_password');
...
function deleteReview(idx) {
  ...
  if (reviews.length === 0) localStorage.removeItem('test_reviews');
  else localStorage.setItem('test_reviews', JSON.stringify(reviews));
  renderReviews();
  showToast('✓ Deleted');
}
...
document.addEventListener('DOMContentLoaded', () => {
  if (sessionStorage.getItem('test_admin')) showPanel();
